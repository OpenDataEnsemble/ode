 ...
 * @property {Record<string, unknown>} params - Host parameters for the formplayer WebView. Put **field prefills** in `params.defaultData` (a plain object). Top-level keys are reserved for bridge/UI: `defaultData`, `theme`, `darkMode`, `themeColors`, `locale` (Formplayer chrome / ODE UI catalogs), `formLocale` (`'default'` or a BCP-47 tag for embedded `ui.json` translations), `profileId` (host-owned storage identity), `context` (session context)—do not rely on `locale` or `formLocale` being copied from params into prefill data. If `defaultData` is omitted, legacy behavior copies every other top-level key as prefill data (still excluding the reserved keys above). On load and submit, when the JSON Schema has non-empty root `properties`, the formplayer keeps only those property keys plus optional observation metadata keys such as schema-declared `locale` and host-stamped `formLocale` (the form translation locale used for that session).
...
/** Synchronous, profile-scoped subset of browser localStorage. */
export interface ProfileLocalStorage {
  getItem(key: string): string | null;
  setItem(key: string, value: string): void;
  removeItem(key: string): void;
  /** Remove only this profile's app keys, not Formplayer or unrelated storage. */
  clear(): void;
}

/**
 * Interface for the Formulus app methods injected into custom-app and Formplayer WebViews.
 * @namespace formulus
 */
export interface FormulusInterface {
   ...
   */
  getLocalStorageRef(): ProfileLocalStorage;
  /**
 ...
 */
export const FORMULUS_INTERFACE_VERSION = '1.6.0';
