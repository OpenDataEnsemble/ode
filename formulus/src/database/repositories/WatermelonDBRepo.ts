...
export class WatermelonDBRepo implements LocalRepoInterface {
  ...
  constructor(database: Database) {
    ...
    void this.ensureColumnIndexes().catch(error => {
      logger.warn(
        'db',
        error instanceof Error ? error.message : 'Column indexes unavailable',
      );
    });
  }
  ...
  private ensureColumnIndexes(): Promise<void> {
    if (!this.columnIndexesPromise) {
      this.columnIndexesPromise = profileActivity.run(
        'Prepare observation column indexes',
        async () => {
          try {
  ...
  async queryObservations(options: {
    ...
    return profileActivity.run('Database: queryObservations', async () => {
      try {
  ...
  async getActiveObservations(): Promise<Observation[]> {
    return profileActivity.run('Database: getActiveObservations', async () => {
      await this.ensureColumnIndexes();
  ...
  async listObservationsPage(
    ...
    return profileActivity.run('Database: listObservationsPage', async () => {
      await this.ensureColumnIndexes();
      ...
      // Drain both reads on failure before releasing the profile activity.
      const [listResult, countResult] = results;
      if (listResult.status === 'rejected') throw listResult.reason;
      if (countResult.status === 'rejected') throw countResult.reason;
      const rawRows = listResult.value;
  ...
  async getAllObservations(): Promise<Observation[]> {
    return profileActivity.run('Database: getAllObservations', async () => {
      try {
  ...
  async applyServerChanges(
    ...
    return profileActivity.run('Database: applyServerChanges', async () => {
      if (!changes.length) {
        return 0;
      }
  ...
  getPendingChanges(): Promise<Observation[]> {
    return profileActivity.run('Database: getPendingChanges', async () => {
  ...
  async markObservationsAsSynced(ids: string[]): Promise<void> {
    return profileActivity.run(
      'Database: markObservationsAsSynced',
      async () => {
        if (ids.length === 0) {
          return;
        }
  ...
  async synchronize(
    ...
    return profileActivity.run('Database: synchronize', async () => {
      try {
