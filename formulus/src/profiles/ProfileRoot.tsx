...
import {
  ...
  subscribeProfileTransition,
} from './ProfileTransitions';
...
export default function ProfileRoot(): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    void bootstrapProfileApp()
      ...
      .catch((failure: unknown) => {
        ...
        if (!cancelled)
          setError(
            code === 'E_PROFILE_COLD_LAUNCH_REQUIRED' ? 'cold' : 'bootstrap',
          );
        console.error(
          'Profile bootstrap stopped; local data has not been reset.',
        );
      });
    return () => {
      cancelled = true;
  ...
  if (AppComponent && !unmounted && transition === 'idle' && !error)
    return <AppComponent />;
  const closeRequired = error === 'cold';
