...
export const LEGACY_PROFILE_KEYS = new Set([
  '@settings', '@server_url', '@token', '@refreshToken', '@tokenExpiresAt', '@user',
  '@last_seen_version', '@last_attachment_version', '@repository_generation', '@lastSync', '@appVersion',
  '@deferred_attachment_downloads', '@ode/adaptivePullPageSize', '@ode/adaptivePushBatchSize',
  '@attachments_layout_v2',
]);

export const LEGACY_DIRECTORIES = ['attachments', 'app', 'forms', 'signatures'] as const;

export function isLegacyProfileKey(key: string): boolean {
  return LEGACY_PROFILE_KEYS.has(key) || key.startsWith('@ode_sequence:') || key.startsWith('@observations');
}

/** Move one entry at a time; never overwrite an unverified destination after a crash. */
async function migrateDirectory(source: string, destination: string): Promise<void> {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      if (await RNFS.exists(target)) {
        ...
        if (sourceHash !== targetHash) throw new Error('Conflicting legacy profile files; migration stopped without overwriting data');
  ...
  if ((await RNFS.readDir(source)).length) throw new Error('Legacy directory changed during migration');
  await RNFS.unlink(source);
}

export async function migrateLegacyProfile(id: string, hasServerUrl: boolean): Promise<void> {
  const root = profileRootFor(id);
  for (const directory of LEGACY_DIRECTORIES) {
    await migrateDirectory(`${RNFS.DocumentDirectoryPath}/${directory}`, `${root}/${directory}`);
  }
  ...
  for (const [key, value] of entries) {
    ...
    if (!hasServerUrl && ['@token', '@refreshToken', '@tokenExpiresAt', '@user'].includes(key)) continue;
    const destination = prefix + key;
    const existing = await AsyncStorage.getItem(destination);
    if (existing !== null && existing !== value) throw new Error('Conflicting legacy profile storage; migration stopped');
    await AsyncStorage.setItem(destination, value);
    if ((await AsyncStorage.getItem(destination)) !== value) throw new Error('Profile storage migration verification failed');
  }
  ...
  if (credentials && hasServerUrl) {
    ...
    if (existing && (existing.username !== credentials.username || existing.password !== credentials.password)) throw new Error('Conflicting migrated credentials');
    const saved = await Keychain.setGenericPassword(credentials.username, credentials.password, options);
    if (!saved) throw new Error('Unable to migrate profile credentials');
    const verified = await Keychain.getGenericPassword(options);
    if (!verified || verified.username !== credentials.username || verified.password !== credentials.password) throw new Error('Credential migration verification failed');
  }
  ...
  if (credentials) {
    ...
    if (await Keychain.getGenericPassword()) throw new Error('Unable to retire legacy credentials');
  }
