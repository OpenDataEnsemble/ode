...
const ProfileStorage = {
  getItem: (key: string): Promise<string | null> =>
    profileActivity.run('Read profile storage', () =>
      AsyncStorage.getItem(prefix() + key),
    ),
  setItem: (key: string, value: string): Promise<void> =>
    profileActivity.run('Write profile storage', () =>
      AsyncStorage.setItem(prefix() + key, value),
    ),
  removeItem: (key: string): Promise<void> =>
    profileActivity.run('Remove profile storage', () =>
      AsyncStorage.removeItem(prefix() + key),
    ),
  multiGet: (
    keys: readonly string[],
  ): Promise<readonly [string, string | null][]> =>
    profileActivity.run('Read profile storage', async () => {
      const scope = prefix();
      const values = await AsyncStorage.multiGet(keys.map(key => scope + key));
      return values.map(
        ([key, value]) =>
          [key.slice(scope.length), value] as [string, string | null],
      );
    }),
  multiSet: (entries: readonly (readonly [string, string])[]): Promise<void> =>
    profileActivity.run('Write profile storage', () => {
      ...
      return AsyncStorage.multiSet(
        entries.map(([key, value]) => [scope + key, value]),
      );
    }),
  ...
  getAllKeys: (): Promise<string[]> =>
    profileActivity.run('List profile storage', async () => {
      const scope = prefix();
  ...
  clear: (): Promise<void> =>
    profileActivity.run('Clear profile storage', async () => {
      const scope = prefix();
