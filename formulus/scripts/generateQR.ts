...
function parseArgs(): FRMLS {
  ...
  if (!params.url || !params.user || !params.pass) {
    console.error('Usage: npm run generate_qr url=<server_url> user=<username> pass=<password>');
    console.error('Example: npm run generate_qr url=http://localhost:3000/synk user=admin pass=admin');
    process.exit(1);
  }
