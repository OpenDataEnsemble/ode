...
} from './SignatureQuestionRenderer';
...
const schema = {
  type: 'object',
  properties: {
    signature: { type: 'object', format: 'signature', title: 'Signature' },
  },
};
...
describe('signature profile-safe preview', () => {
  it('resolves a migrated legacy basename through the bridge without changing stored data', async () => {
    const value = {
      ...
      uri: 'file:///old/Documents/signatures/signature_123.png',
    };
    const snapshot = { ...value };
    const resolve = vi
      .spyOn(FormulusClient.getInstance(), 'getAttachmentUri')
      .mockResolvedValue(
        'file:///docs/profiles/active/signatures/signature_123.png',
      );
    render(<Form value={value} />);
    await waitFor(() =>
      expect(screen.getByAltText('Signature')).toHaveAttribute(
        'src',
        'file:///docs/profiles/active/signatures/signature_123.png',
      ),
    );
  ...
  it.each([
    'file:///docs/profiles/other/signatures/private.png',
    'https://untrusted.example/signature.png',
    'data:image/svg+xml;base64,PHN2Zz4=',
  ])('never uses the stored URI as a fallback: %s', async uri => {
    const resolve = vi
      .spyOn(FormulusClient.getInstance(), 'getAttachmentUri')
      .mockResolvedValue(null);
    render(
      <Form value={{ type: 'signature', filename: 'missing.png', uri }} />,
    );
  ...
  it('does not load a URI-only legacy object', () => {
    const resolve = vi.spyOn(FormulusClient.getInstance(), 'getAttachmentUri');
    render(
      <Form value={{ type: 'signature', uri: 'file:///arbitrary.png' }} />,
    );
    expect(screen.queryByAltText('Signature')).toBeNull();
    expect(resolve).not.toHaveBeenCalled();
  });
  ...
  it('ignores stale resolution when another signature is shown', async () => {
    ...
    vi.spyOn(FormulusClient.getInstance(), 'getAttachmentUri')
      .mockReturnValueOnce(first)
      .mockResolvedValueOnce('file:///active/signatures/new.png');
    const view = render(
      <Form
        value={{
          type: 'signature',
    ...
    await waitFor(() =>
      expect(screen.getByAltText('Signature')).toHaveAttribute(
        'src',
        'file:///active/signatures/new.png',
      ),
    );
    await act(async () => {
      finish('file:///active/signatures/old.png');
    });
    expect(screen.getByAltText('Signature')).toHaveAttribute(
      'src',
      'file:///active/signatures/new.png',
