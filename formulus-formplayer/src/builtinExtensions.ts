...
export async function getDynamicChoiceList(
  ...
  try {
    ...
    if (usesAgeFromDob && whereClause) {
      ...
      const agePattern = /(NOT\s+)?age_from_dob\([^)]+\)\s*(>=|<=|>|<|=|!=)\s*\d+/gi;
      ...
      for (let i = 0; i < parts.length; i += 2) {
        ...
        const cleanCondition = condition.replace(/^NOT\s+/i, '').replace(/^\(+|\)+$/g, '').trim();
        if (cleanCondition && !agePattern.test(condition) && !agePattern.test(cleanCondition)) {
    ...
    if (usesAgeFromDob && originalWhereClause) {
      ...
      const ageConditionPattern = /(NOT\s+)?age_from_dob\(([^)]+)\)\s*(>=|<=|>|<|=|!=)\s*(\d+)/gi;
        ...
        try {
          ...
          const evaluateCondition = (condition: typeof ageConditions[0]): boolean => {
            ...
            switch (condition.operator) {
              case '>=': result = age >= condition.threshold; break;
              case '<=': result = age <= condition.threshold; break;
              case '>': result = age > condition.threshold; break;
              case '<': result = age < condition.threshold; break;
              case '=': result = age === condition.threshold; break;
          ...
          if (ageConditions.length === 1) {
            ...
            for (let i = 0; i < ageConditions.length; i++) {
              ...
              if (i < ageConditions.length - 1) {
                ...
                const betweenText = originalWhereClause.substring(
                  condition.position + (originalWhereClause.substring(condition.position).match(/age_from_dob\([^)]+\)\s*(>=|<=|>|<|=|!=)\s*\d+/i)?.[0]?.length || 0),
            ...
            const beforeFirst = originalWhereClause.substring(0, firstConditionStart);
            const lastCondition = ageConditions[ageConditions.length - 1];
            const lastConditionEnd = lastCondition.position + 
              (originalWhereClause.substring(lastCondition.position).match(/age_from_dob\([^)]+\)\s*(>=|<=|>|<|=|!=)\s*\d+/i)?.[0]?.length || 0);
            ...
            const hasOpeningParen = /\(\s*$/.test(beforeFirst.trim().slice(-5));
            ...
            if (logics.length === 0) {
              ...
              if (isGrouped && logics[0] === 'AND' && logics.some(l => l === 'OR')) {
                ...
                if (orIndex > 0) {
                  ...
                  const groupedResult = results.slice(0, orIndex + 1).every(r => r);
                  // Then OR with remaining conditions: OR C
                  const remainingResults = results.slice(orIndex + 1);
                  finalResult = groupedResult || remainingResults.some(r => r);
                } else {
