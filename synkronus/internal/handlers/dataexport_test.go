...
func TestHandler_RawJSONExportHandler(t *testing.T) {
	tests := []struct {
		name           string
		setupMock      func(*mocks.MockDataExportService)
		expectedStatus int
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			req := httptest.NewRequest(http.MethodGet, "/api/dataexport/raw-json", nil)
			w := httptest.NewRecorder()
			h.RawJSONExportHandler(w, req)
			...
			if !tt.expectError {
				disposition := w.Header().Get("Content-Disposition")
				expectedDisposition := "attachment; filename=\"observations_raw_json.zip\""
				if disposition != expectedDisposition {
					t.Errorf("Expected Content-Disposition %s, got %s", expectedDisposition, disposition)
				}
			}
			if !tt.expectError && len(w.Body.Bytes()) == 0 {
				t.Error("Expected response body but got empty content")
			}
		})
	}
